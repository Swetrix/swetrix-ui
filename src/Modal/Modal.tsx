...
export interface IModalProps {
    ...
    size?: 'regular' | 'large'
    customButtons?: Array<React.ReactNode> | React.ReactNode
}
        ...
        <Transition.Root show={isOpened} as={Fragment}>
          ...
          >
            <div className='flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0'>
              ...
              >
                <Dialog.Overlay
                    className='fixed inset-0 bg-gray-500 bg-opacity-75 dark:bg-opacity-55 transition-opacity'/>
              </Transition.Child>
              ...
              >
                ...
                >
                  <div className='sm:flex sm:items-start'>
                    {type === 'success' && (
                        <div
                            className='sm:mr-3 mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-green-100 sm:h-10 sm:w-10'>
                          <CheckIcon className='h-6 w-6 text-green-600' aria-hidden='true'/>
                        </div>
                    )}
                    {type === 'error' && (
                        <div
                            className='sm:mr-3 mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-red-100 sm:h-10 sm:w-10'>
                          <ExclamationTriangleIcon className='h-6 w-6 text-red-600' aria-hidden='true'/>
                        </div>
                    )}
                    {type === 'info' && (
                        <div
                            className='sm:mr-3 mx-auto flex-shrink-0 flex items-center text-center justify-center h-12 w-12 rounded-full bg-blue-100 sm:h-10 sm:w-10'>
                          <InformationCircleIcon className='h-6 w-6 text-blue-600' aria-hidden='true'/>
                        </div>
                    )}
                    {type === 'warning' && (
                        <div
                            className='sm:mr-3 mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-orange-100 sm:h-10 sm:w-10'>
