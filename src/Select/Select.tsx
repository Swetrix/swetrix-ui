...
export interface ISelectProps {
    ...
    iconExtractor?: (item: any, index: number) => React.ReactNode,
    keyExtractor?: (item: any, index: number) => string,
}

export const Select: React.FC<ISelectProps> = memo(
    ({
         title, label, items, labelExtractor, keyExtractor, iconExtractor, onSelect,
     }) => (
        <Listbox value={title}
                 onChange={onSelect}
        >
            {({open}) => (
                <>
                    <Listbox.Label
                        className='block text-sm whitespace-pre-line font-medium text-gray-700 dark:text-gray-100'>{label}</Listbox.Label>
                    <div className='mt-1 relative'>
                        <Listbox.Button
                            className='relative w-full bg-white border border-gray-300 dark:text-gray-50 dark:border-gray-800 dark:bg-gray-700 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm'>
